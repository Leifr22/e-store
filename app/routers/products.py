...
async def product_by_category(db: Annotated[Session, Depends(get_db)], category_slug: str):
    ...
    subcategories=db.scalar(select(Category).where(Category.slug == category_slug))
    cat_and_sub=[category.id]+[i.id for i in subcategories]
    product_categories = db.scalars(select(Product).where(Product.category_id.in_(cat_and_sub), Product.is_active==True, Product.stock > 0)).all()
