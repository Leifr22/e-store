...
async def create_category(db: Annotated[AsyncSession, Depends(get_db)],
    ...
    if get_user.get('is_admin'):
        ...
        raise HTTPException(
            ...
            detail='You must have admin rights'
        )
...
async def update_category(db: Annotated[AsyncSession, Depends(get_db)], category_id: int,
    ...
    if get_user.get('is_admin'):
        ...
        if update_category.parent_id != 0:
            parent_category = await db.scalar(select(Category).where(Category.id == update_category.parent_id))
            if parent_category is None:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail=f'Parent category with id {update_category.parent_id} does not exist.'
                )
